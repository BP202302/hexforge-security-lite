...
from .config import FETCH_TEXT_MAX_BYTES, MAX_BODY_BYTES, REQUEST_TIMEOUT, USER_AGENT
...
def fetch_text(url: str, timeout: int = REQUEST_TIMEOUT, max_bytes: int = FETCH_TEXT_MAX_BYTES) -> Tuple[int, Dict[str, str], str]:
...
def fetch_method(url: str, method: str = "OPTIONS", timeout: int = REQUEST_TIMEOUT, max_bytes: int = 2048) -> Tuple[int, Dict[str, str], str]:
    safe_method = method.upper().strip()
    if safe_method not in {"OPTIONS", "HEAD"}:
        raise ValueError("HexForge Lite fetch_method only supports safe OPTIONS/HEAD probes.")
    request = Request(url, headers={"User-Agent": USER_AGENT}, method=safe_method)
    try:
        with urlopen(request, timeout=timeout) as response:
            body = response.read(max_bytes) if safe_method != "HEAD" else b""
            text = _decode_body(body, response.headers) if body else ""
            return getattr(response, "status", 200), dict(response.headers.items()), text
    except HTTPError as exc:
        body = exc.read(max_bytes) if safe_method != "HEAD" else b""
        text = _decode_body(body, exc.headers) if body else ""
