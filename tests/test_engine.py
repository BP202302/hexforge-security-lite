...
LAB_PAGES = [
    ...
    },
    {
        ...
        "js_body": "fetch('/api/profile'); fetch('/rest/products/search'); const font='/v18/pxiKyp0ihIEF2isQFJXGdg.woff2'; const access_token = null; localStorage.getItem('theme');",
    },
    {
    ...
    {
        "name": "http_methods_trace",
        "url": "https://lab.local/options",
        "headers": {"Content-Type": "text/html"},
        "html": "<html><body>options test</body></html>",
        "methods": "GET, POST, OPTIONS, TRACE",
    },
    {
        "name": "plugin_generator_metadata",
        "url": "https://lab.local/plugin",
        "headers": {"Content-Type": "text/html"},
        "html": "<html><head><meta name='generator' content='HexForgeFixture 1.0'></head></html>",
    },
]
...
class EnginePrecisionTests(unittest.TestCase):
    def run_case(self, case: dict) -> dict:
        ...
        def fake_fetch_method(url: str, method: str = "OPTIONS", timeout: int = 5, max_bytes: int = 2048):
            if case.get("methods"):
                return 204, {"Allow": case["methods"]}, ""
            return 204, {}, ""
        ...
        original_http_methods_fetch_method = http_methods_module.fetch_method
        original_js_surface_fetch_text = js_surface_module.fetch_text
        ...
        try:
            ...
            http_methods_module.fetch_method = original_http_methods_fetch_method
            js_surface_module.fetch_text = original_js_surface_fetch_text
    ...
    def test_all_10_lab_profiles_return_valid_reports(self):
        for case in LAB_PAGES:
            with self.subTest(case=case["name"]):
                ...
                self.assertEqual(report["version"], "1.9.0-stable")
                self.assertIn("summary", report)
                self.assertGreaterEqual(len(report["modules"]), 20)
    ...
    def test_js_surface_analyzer_maps_reviewable_js(self):
        report = self.run_case(next(case for case in LAB_PAGES if case["name"] == "client_surface_app"))
        js_items = [f for f in report["findings"] if f["module"] == "js_surface_analyzer"]
        self.assertTrue(js_items)
        evidence = "\n".join(f["evidence"] for f in js_items)
        self.assertIn("/api/profile", evidence)
        self.assertIn("js_surface_analyzer", report["modules"])
        surface = report.get("surface_map", {}).get("js_surface", {})
        self.assertIn("/api/profile", "\n".join(surface.get("endpoints", [])))
    ...
    def test_http_methods_safe_probe_flags_trace_as_review(self):
        report = self.run_case(next(case for case in LAB_PAGES if case["name"] == "http_methods_trace"))
        method_findings = [f for f in report["findings"] if f["module"] == "http_methods"]
        self.assertTrue(method_findings)
        self.assertTrue(any(f["id"] == "HF-LITE-039" for f in method_findings))
        self.assertIn("TRACE", "\n".join(f["evidence"] for f in method_findings))

    def test_lite_plugin_loader_executes_safe_plugin(self):
        report = self.run_case(next(case for case in LAB_PAGES if case["name"] == "plugin_generator_metadata"))
        plugin_items = [f for f in report["findings"] if f["id"] == "HF-PLUGIN-001"]
        self.assertTrue(plugin_items)
        self.assertEqual(plugin_items[0]["severity"], "info")
        self.assertIn("plugin:generator_metadata_plugin", report["modules"])

    def test_datasets_are_loaded_by_runtime(self):
        from hexforge_lite.datasets import headers_dataset, severity_profile
        self.assertIn("content-security-policy", headers_dataset().get("required", []))
        self.assertGreater(severity_profile().get("high", 0), severity_profile().get("low", 0))
...
class ServerGuardrailTests(unittest.TestCase):
    ...
    def test_waitlist_requires_valid_email_and_consent(self):
        from api.waitlist import waitlist_response
        bad = waitlist_response({"email": "not-an-email", "consent": True})
        self.assertFalse(bad["ok"])
        no_consent = waitlist_response({"email": "user@example.com", "consent": False})
        self.assertFalse(no_consent["ok"])
