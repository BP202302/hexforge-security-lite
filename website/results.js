...
const severityDonut = document.getElementById('severityDonut');
...
function renderSeverityChart(data) {
  if (!severityDonut || !severityLegend) return;
  ...
  const total = segments.reduce((sum, item) => sum + item[1], 0) || (data.findings || []).length || 0;
  const colors = {
    critical: '#ff3b5c',
    high: '#ff7187',
    medium: '#f7c76b',
  ...
  if (!total) {
    severityDonut.style.background = 'conic-gradient(rgba(255,255,255,0.08) 0 360deg)';
    severityDonut.innerHTML = `<span>0<small>${escapeHtml(t('findings'))}</small></span>`;
    severityLegend.innerHTML = '';
    return;
  }
  let cursor = 0;
  const slices = segments.map(([name, count]) => {
    const degrees = (count / total) * 360;
    const start = cursor;
    cursor += degrees;
    return `${colors[name]} ${start}deg ${cursor}deg`;
  }).join(', ');
  severityDonut.style.background = `conic-gradient(${slices})`;
  severityDonut.innerHTML = `<span>${escapeHtml(total)}<small>${escapeHtml(t('findings'))}</small></span>`;
  severityLegend.innerHTML = segments.map(([name, count]) => `
    <div class="severity-legend-item">
      <i style="background:${colors[name]}"></i>
      <span>${escapeHtml(translateSeverity(name))}</span>
...
function renderSurfaceMap(data) {
  ...
  const blocks = [
    ...
    {
      title: 'JS Surface',
      subtitle: `${t('scriptsLabel')}: ${(surface.js_surface?.inspected_scripts || []).length}`,
      body: renderTags([...(surface.js_surface?.endpoints || []), ...(surface.js_surface?.sensitive_names_redacted || []), ...(surface.js_surface?.browser_storage || [])]),
    },
  ];
...
function demoReport() {
  return {
    ...
    summary: {
      ...
      severity_counts: { high: 0, medium: 2, low: 2, info: 2 },
    },
    surface_map: {
      ...
      js_surface: {
        inspected_scripts: ['/main.js'],
        endpoints: ['/api/profile', '/graphql'],
        sensitive_names_redacted: ['access_token'],
    ...
    findings: [
      ...
      {
        id: 'HF-LITE-040', severity: 'low', confidence: 'medium', kind: 'Review',
        title: 'JavaScript surface exposes reviewable endpoints',
        description: 'Same-origin JavaScript references endpoint-like paths.',
        location: 'Same-origin JavaScript bundle analysis',
        evidence: 'JS endpoints: /api/profile | /graphql',
        recommendation: 'Review paths only within authorized scope and verify auth manually.',
        precision_note: 'Lite reads referenced same-origin scripts only; no fuzzing or payloads.',
      },
      {
...
async function run() {
  ...
  if (copyJsonBtn) {
    copyJsonBtn.addEventListener('click', async () => {
      ...
      const previous = copyJsonBtn.textContent;
      copyJsonBtn.textContent = 'Copied JSON';
      setTimeout(() => { copyJsonBtn.textContent = previous; }, 1400);
    });
  }
  if (demo) {
    draw(demoReport());
  ...
  if (params.get('from') === 'session') {
    const stored = sessionStorage.getItem('hf_last_report');
    if (stored) {
      draw(JSON.parse(stored));
      return;
    }
  }
