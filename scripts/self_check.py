...
def run_case(case):
    ...
    def fake_fetch_text(_url, timeout=5, max_bytes=1200):
        ...
        if _url.endswith("/app.js"):
            return 200, {"Content-Type":"application/javascript"}, "fetch('/api/profile'); fetch('/rest/products/search'); const font='/v18/pxiKyp0ihIEF2isQFJXGdg.woff2';"
    ...
    old_fetch, old_text, old_tls, old_client_text, old_security_text = scanner_module.fetch_url, robots_module.fetch_text, tls_module.tls_summary, client_surface_module.fetch_text, security_txt_module.fetch_text
...
for pass_no in range(1, 4):
    ...
    for case in LAB_PAGES:
        ...
        assert_true(report["version"] == "1.8.5-community", "version mismatch")
        assert_true(len(report["modules"]) == 17, "module count mismatch")
...
print("[HexForge] self-check complete: 12 lab profiles passed x3", flush=True)
os._exit(0)
